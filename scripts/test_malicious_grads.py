...
def train(
    ...
    for check_type in metrics_log[trainer_type].keys():
        if (
            check_type == "random_direction_projection_full"
            or check_type == "gradient_norms_full"
        ):
            gradients = torch.cat(
                [
                    param.grad.detach().cpu().clone().view(-1)
                    if param.grad is not None
...
def main():
    ...
    optimizers = {
        t: AdamW(model_templates[t].parameters(), lr=5e-5) for t in model_templates
    }
    ...
    for epoch in epochs:
        ...
        test_layer_indices = [
            i
            for i, layer in enumerate(model_templates["normal_small"].parameters())
            if len(layer.size()) == 1
        ]
        indices = random.choice(test_layer_indices)
        model_example = next(iter(model_templates.values()))  # Get an example model
        example_param = next(
            islice(model_example.parameters(), indices, None)
        )  # Get the specific parameter for layer-wise
        ...
        total_grad_size = sum(
            p.numel() for p in model_example.parameters() if p.requires_grad
        )
        ...
        metrics_log = {
            trainer_type: {
                ...
                for check_type in [
                    "sum_gradients",
                    "random_direction_projection_layer",
                    "random_direction_projection_full",
                    "gradient_norms",
                    "gradient_norms_full",
                    "loss",
                    "perplexity",
        ...
        for trainer_type, model in model_templates.items():
            ...
            dataloader = SubsetFalconLoader(
                batch_size=batch_sizes[trainer_type], sequence_length=1024, rows=group
            )
            ...
            train(
                ...
                metrics_log,
                indices,
                mimic_models=mimic_models if trainer_type == "idle" else None,
                z_layer=z_layer_wise,
                z_full=z_full_gradient,
            )
