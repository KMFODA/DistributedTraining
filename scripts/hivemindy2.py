...
class DTAverager(hivemind.DecentralizedAverager):
    ...
    def step(
        ...
        wait: bool = True,
        custom_group_info: Optional[
            GroupInfo
        ] = None,  # New parameter to accept custom GroupInfo
            ...
            while not step.done():
                try:
                    ...
                    with self._register_allreduce_group(custom_group_info):
                        print("Here yeeeeeeee", custom_group_info)
...
class DTGradientAverager(DTAverager):
    ...
    def step(
        ...
        **kwargs,
        ...
        return super().step(
            wait=False, require_trigger=False, allow_retries=False, **kwargs
        )
...
def load_state_from_peer(self):
    ...
    self.state_averager.load_final_state_from_peers(
        self.tracker.global_progress.epoch,
        timeout=self.state_averager.next_chunk_timeout,
    )
